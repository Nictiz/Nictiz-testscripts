...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns:nf="http://www.nictiz.nl/functions"
    xmlns:f="http://hl7.org/fhir" xmlns:nts="http://nictiz.nl/xsl/testscript"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl"
    xmlns:util="urn:hl7:utilities" version="2.0" xmlns=""
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xmlns:xs="http://www.w3.org/2001/XMLSchema">
    <!--Import mp specific constants (and package for underlying imports)-->
    <xsl:import
        href="../../../../../YATC-internal/ada-2-fhir-r4/env/mp/9.3.0/payload/mp9_latest_package.xsl"/>
    ...
    <xsl:param name="mappingsUrl4FhirFixtures"
        >../../../../../HL7-mappings/master/ada_2_fhir-r4/mp/9.3.0/touchstone/test/4touchstone_mp</xsl:param>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="$adaTransaction">
            ...
            <xsl:variable name="fileName"
                select="replace($adaTransId, '.+(tst|kwal)-(.+)-v30', '$2')"/>
            <xsl:variable name="fhirFixture"
                select="document(concat($mappingsUrl4FhirFixtures, '/', $adaTransId, '.xml'))"/>
            ...
            <xsl:variable name="scenarioset">
                <xsl:choose>
                    <xsl:when test="string-length(scenario-nr/@value) gt 0">
                        <xsl:value-of
                            select="replace(scenario-nr/@value, '(\d+)\.?(\d*[a-z]?)\*?\s?.*', '$1')"
                        />
                    </xsl:when>
                    <xsl:when
                        test="string-length(voorstel_gegevens/(voorstel | antwoord)/identificatie/@value) gt 0">
                        <xsl:value-of
                            select="lower-case(nf:assure-logicalid-chars(voorstel_gegevens/(voorstel | antwoord)/identificatie/@value))"
                        />
                    </xsl:when>
                </xsl:choose>
            </xsl:variable>
            <xsl:variable name="scenarioSub"
                select="replace(scenario-nr/@value, '(\d+)\.?(\d*[a-z]?)\*?\s?.*', '$2')"/>
            ...
            <xsl:variable name="testScriptString" as="element()*">
                <xsl:choose>
                    <xsl:when
                        test="self::beschikbaarstellen_medicatiegegevens | self::sturen_medicatiegegevens">
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:call-template name="util:logMessage">
                            ...
                            <xsl:with-param name="msg">Ada transaction '<xsl:value-of
                                    select="local-name()"/>' with transaction type '<xsl:value-of
                                    select="$transactionType"/>' not properly supported
                                yet.</xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:choose>
                ...
                <xsl:when
                    test="self::beschikbaarstellen_medicatiegegevens and normalize-space(upper-case($transactionType)) = ('RETRIEVE', 'SERVE')">
                    <xsl:call-template name="util:logMessage">
                        <xsl:with-param name="level" select="$logERROR"/>
                        <xsl:with-param name="msg">Pull medication data should call a separate
                            XSLT</xsl:with-param>
                ...
                <xsl:when test="normalize-space(upper-case($transactionType)) = ('SEND', 'RECEIVE')">
                    ...
                    <xsl:variable name="includeNumResources" as="element()*">
                        <xsl:for-each-group select="$fhirFixture/f:Bundle/f:entry/f:resource/f:*"
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when
                                    test="current-grouping-key() = ('MedicationRequest', 'MedicationDispense', 'MedicationStatement', 'MedicationAdministration', 'Medication')">
                                    <nts:include value="assert.request.numResources" scope="common"
                                        resource="{current-grouping-key()}"
                    ...
                    <xsl:variable name="idString"
                        select="replace(concat('mp9-', $testScriptString/@short, '-', normalize-space(lower-case($transactionType)), '-', $scenarioset, '-', $scenario), '(.*?)-?(-$)', '$1')"/>
                    <xsl:variable name="testId"
                        select="concat('scenario', $scenarioset, '-', $scenario, '-', lower-case($transactionType), '-', $testScriptString/@short)"/>
                    <xsl:choose>
                        <!-- Receive -->
                        <xsl:when test="$ntsScenario = 'server'">
                            <TestScript xmlns="http://hl7.org/fhir"
                                xmlns:nts="http://nictiz.nl/xsl/testscript"
                                nts:scenario="{$ntsScenario}">
                                <id value="{$idString}"/>
                                <version value="r4-mp9-3.0.0-beta"/>
                                <name
                                    value="MP9 - {nf:first-cap($ntsScenario)} - Scenario {$scenarioset}.{$scenario} - {nf:first-cap($transactionType)} {$testScriptString/@long} - {$fileName}"/>
                                <description
                                    value="Scenario {$scenarioset}.{$scenario} - {nf:first-cap($transactionType)} {$testScriptString/@long} for {$fixturePatient/f:name/f:text/@value}."/>
                                <nts:fixture id="{$adaTransId}"
                                    href="fixtures/{$adaTransId}.{'{$_FORMAT}'}"/>
                                <nts:includeDateT value="yes"/>
                                <!--<xsl:apply-templates select="$deleteStuff/f:variable" mode="Nictiz-intern"/>-->
                                <test id="{$testId}">
                                    <name value="Scenario {$scenarioset}.{$scenario}"/>
                                    <description
                                        value="{nf:first-cap($transactionType)} {$testScriptString/@long} in a transaction Bundle"/>
                                    <action>
                                        <operation>
                                            <type>
                                                <system
                                            ...
                                            <description
                                                value="Test server to handle a Bundle of type transaction."/>
                                    ...
                                    <nts:include value="assert.response.bundleContent"
                                        scope="common" bundleType="transaction-response"/>
                                ...
                                <test id="{$testId}">
                                    ...
                                    <action>
                                        <operation>
                                            ...
                                            <description
                                                value="Test client to POST a Bundle of type transaction."/>
                                            <destination value="1"/>
                                            <origin value="1"/>
                                    ...
                                    <nts:include value="test.client.successfulTransaction"
                                        scope="common"/>
                                    <xsl:copy-of select="$includeNumResources"/>
                                </test>
                                <xsl:if test="count($identifyResources) gt 0">
                ...
                <xsl:otherwise>
                    <xsl:call-template name="util:logMessage">
                        ...
                        <xsl:with-param name="msg">Ada transaction '<xsl:value-of
                                select="local-name()"/>' with transaction type '<xsl:value-of
                                select="$transactionType"/>' not supported yet.</xsl:with-param>
                    </xsl:call-template>
