...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns:nf="http://www.nictiz.nl/functions" xmlns:f="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:util="urn:hl7:utilities" version="2.0" xmlns="" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template name="createNts">
        ...
        <xsl:variable name="clCategory" select="nf:makeCLCategory($buildingBlockShort)"/>
        <xsl:variable name="clSubcategory" select="nf:makeCLSubcategory($buildingBlockShort)"/>
            ...
            <xsl:when test="$transactionTypeNormalized = ('retrieve', 'serve')">
                <xsl:choose>
                    ...
                    <xsl:when test="($transactionTypeNormalized = 'retrieve' and $buildingBlockShort != 'CONS') or ($transactionTypeNormalized = 'serve' and not(contains($buildingBlockShort, 'CONS')))">
                        <xsl:result-document href="{concat($outputDirNormalized, $clCategory, '/', $clSubcategory, '/', nf:system-dir($transactionTypeNormalized, $buildingBlockShort), '/', $newFilename)}">
    ...
    <xsl:template name="handleFilterScenario">
        ...
        <xsl:result-document href="{concat($outputDirNormalized, nf:system-dir($transactionTypeNormalized, $buildingBlockShort), '/', $buildingBlockLong, '/', $newFilename)}">
            <xsl:variable name="ntsScenario" as="xs:string?">
                <xsl:choose>
    ...
    <xsl:function name="nf:system-dir" as="xs:string">
        ...
        <xsl:variable name="transactionTypeNormalized" select="normalize-space(lower-case($transactionType))"/>
        <xsl:choose>
            <xsl:when test="$transactionTypeNormalized = 'retrieve'"><xsl:value-of select="concat('Retrieving-System (MP-MGR - ',$buildingBlockShort,')')"/></xsl:when>
            <xsl:when test="$transactionTypeNormalized = 'serve'"><xsl:value-of select="concat('Serving-System (MP-MGB - ',$buildingBlockShort,')')"/></xsl:when>
            <xsl:when test="$transactionTypeNormalized = 'receive'"><xsl:value-of select="concat('Receiving-System (MP-MGO - ',$buildingBlockShort,')')"/></xsl:when>
            <xsl:when test="$transactionTypeNormalized = 'send'"><xsl:value-of select="concat('Sending-System  (MP-MGS - ',$buildingBlockShort,')')"/></xsl:when>
    ...
    <xd:doc>
        <xd:desc>Make Conformancelab category string based on short building block string</xd:desc>
        <xd:param name="buildingBlockShort">short building block string: MA/WDS/VV/TA/MVE/MGB/MTD</xd:param>
    </xd:doc>
    <xsl:function name="nf:makeCLCategory" as="xs:string?">
        <xsl:param name="buildingBlockShort" as="xs:string?"/>
        
        <xsl:choose>
            <!-- consolidation buildingBlockShort is a string like "CONS-MA" -->
            <xsl:when test="contains($buildingBlockShort, 'MA')">Stap 3</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MGB')">Stap 4</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'TA')">Stap 5</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MTD')">Stap 6</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'WDS')">Stap 3</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'VV')">Stap 3</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MVE')">Stap 5</xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="util:logMessage">
                    <xsl:with-param name="level" select="$logFATAL"/>
                    <xsl:with-param name="msg">Could not determine building block: <xsl:value-of select="$buildingBlockShort"/></xsl:with-param>
    ...
    <xd:doc>
        <xd:desc>Make Conformancelab subcategory string based on short building block string</xd:desc>
        <xd:param name="buildingBlockShort">short building block string: MA/WDS/VV/TA/MVE/MGB/MTD</xd:param>
    </xd:doc>
    <xsl:function name="nf:makeCLSubcategory" as="xs:string?">
        <xsl:param name="buildingBlockShort" as="xs:string?"/>
        
        <xsl:choose>
            <!-- consolidation buildingBlockShort is a string like "CONS-MA" -->
            <xsl:when test="contains($buildingBlockShort, 'MA')">MedicatieAfspraak (MA)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MGB')">MedicatieGebruik (MGB)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'TA')">ToedieningsAfspraak (TA)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MTD')">MedicatieToediening (MTD)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'WDS')">Wisselend DoseerSchema (WDS)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'VV')">VerstrekkingsVerzoek (VV)</xsl:when>
            <xsl:when test="contains($buildingBlockShort, 'MVE')">MedicatieVerstrekking (MVE)</xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="util:logMessage">
                    <xsl:with-param name="level" select="$logFATAL"/>
                    <xsl:with-param name="msg">Could not determine building block: <xsl:value-of select="$buildingBlockShort"/></xsl:with-param>
