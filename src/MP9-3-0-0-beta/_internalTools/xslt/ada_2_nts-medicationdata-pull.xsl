...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns:nf="http://www.nictiz.nl/functions" xmlns:f="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" version="2.0" xmlns="" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="collection($uri-collection)">
            ...
            <xsl:call-template name="util:logMessage">
                <xsl:with-param name="level" select="$logINFO"/>
                <xsl:with-param name="msg">1. handling <xsl:value-of select="$adaTransId"/></xsl:with-param>
            </xsl:call-template>
            ...
            <xsl:variable name="buildingBlockShort" select="substring-before(substring-after($adaTransId, concat('mg-mp-mg-', $fileNamePart, '-')), '-')[1]" as="xs:string*"/>
            <xsl:call-template name="util:logMessage">
                <xsl:with-param name="level" select="$logINFO"/>
    ...
    <xsl:template name="createNts">
        ...
        <xsl:variable name="medicationValues" as="xs:string*" select="$adaInstance/medicamenteuze_behandeling//farmaceutisch_product/@value | $adaInstance//medicamenteuze_behandeling_id/..//farmaceutisch_product/@value"/>
