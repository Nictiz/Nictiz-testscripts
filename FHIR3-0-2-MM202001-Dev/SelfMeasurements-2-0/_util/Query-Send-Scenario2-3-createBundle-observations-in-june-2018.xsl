...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="outputid">medmij-selfmeasurements-fhir3-0-1-scenario-2-3-bundle</xsl:variable>
    ...
    <xsl:variable name="observation-fixtures" as="xs:string+">
        ...
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BloodGlucose-kwalificatie4.xml</xsl:text>
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BloodGlucose-kwalificatie5.xml</xsl:text>
        ...
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BloodPressure-kwalificatie3.xml</xsl:text>
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BloodPressure-kwalificatie4.xml</xsl:text>
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BloodPressure-kwalificatie5.xml</xsl:text>
        ...
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BodyWeight-kwalificatie3.xml</xsl:text>
        <xsl:text>medmij-selfmeasurements-fhir3-0-1-BodyWeight-kwalificatie4.xml</xsl:text>
    </xsl:variable>
    <xsl:variable name="entries" as="element(f:Observation)*">
        <xsl:for-each select="$observation-fixtures">
            ...
            <xsl:if test="$xml[self::f:Observation]/f:effectiveDateTime[starts-with(@value, '2018-06')]">
    ...
    <xsl:variable name="allotherentries" as="element()*">
        <xsl:for-each select="$patient-fixtures">
            <xsl:variable name="file" select="."/>
            <xsl:variable name="xml" select="doc($file)/*" as="element()"/>
            <xsl:if test="$xml[self::f:Patient]">
                <xsl:element name="{local-name($xml)}" namespace="http://hl7.org/fhir">
                    <xsl:copy-of select="$xml/*" copy-namespaces="no"/>
                </xsl:element>
            </xsl:if>
